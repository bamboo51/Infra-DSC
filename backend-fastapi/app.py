from fastapi import FastAPI, UploadFile, File, Form, Depends, HTTPException, Query, APIRouter
from fastapi.staticfiles import StaticFiles
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import JSONResponse
from typing import Optional
from utils import create_thumbnail, save_mask, get_full_url

import datetime
import io
from schema import Detection, Segmentation, PhotoResponse, PredictionResponse, ErrorResponse, PhotoListResponse, PhotoWithResults
...
async def predict(
    ...
    logger.info(f"Starting prediction for image: {image.filename}, latitude: {latitude}, longitude: {longitude}")
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Failed to calculate image hash: {str(e)}")
    
    # check for existing image in database
    existing_photo = db.query(Photo).filter(Photo.image_hash == hash_value).first()

    if existing_photo:
        logger.info(f"Found existing photo with ID: {existing_photo.id}, returning cached results.")
        detection_results = [
            {
                "box_x1": d.box_x1,
                "box_y1": d.box_y1,
        ...
        if not detection_results:
            logger.warning(f"No detection results found for existing photo ID: {existing_photo.id}")
        ...
        if not segmentation_results:
            logger.warning(f"No segmentation results found for existing photo ID: {existing_photo.id}")
    ...
    try:
        ...
        logger.info(f"Model inference completed for photo ID: {photo_instance.id} with crack ratio: {crack_ratio}")
        db.commit()
    except Exception as e:
        ...
        if thumbnail_path and os.path.exists(thumbnail_path):
            os.remove(thumbnail_path)
            logger.info(f"Removed thumbnail at {thumbnail_path} due to error during inference.")
    ...
    try:
        ...
        for i, seg in enumerate(segmentation_results):
            ...
            seg_instance = SegmentationModel(
                ...
                mask_uri=save_mask(seg["mask_uri"], photo_instance.id, i, seg["class_name"])
            )
            db.add(seg_instance)
        ...
        if thumbnail_path and os.path.exists(thumbnail_path):
            os.remove(thumbnail_path)
            logger.info(f"Removed thumbnail at {thumbnail_path} due to error during result save.")
...
async def list_photos(
    ...
    photo_responses = [
        PhotoResponse(
            ...
            coords={"latitude": photo.latitude, "longitude": photo.longitude} if photo.latitude and photo.longitude else None
        ) for photo in photos
    ]
...
@router.get("/photos/{id}/", response_model=PhotoWithResults, responses={
    404:{"model": ErrorResponse, "description": "Photo not found"}
})
async def get_photo_details(
    ...
    segmentations = db.query(SegmentationModel).filter(SegmentationModel.photo_id == id).all()
